...
> ⚠️ **重要规则：回复用户权限状态之前，必须先自己调用工具查看真实权限，确认后再回复。** 飞书 app 通常有 100+ 个 scope，你必须通过 `feishu_app_scopes` 获取完整列表并搜索确认，不要凭猜测或直觉判断。如果你不确定，**默认假设权限存在**，直接尝试调用 API——调用失败再报缺失，比误报缺失体验好得多。
...
#### 检测项 2: 飞书消息读取能力
- 调用 `feishu_app_scopes` 获取完整权限列表
- 在返回的 granted 数组中搜索 `im:message:readonly`（注意：scope 列表可能很长，有 100+ 个，必须完整遍历，不要只看前几个就判断缺失）
- 只要 granted 中存在 name 为 `im:message:readonly` 的条目（无论 type 是 tenant 还是 user） → ✅ 飞书消息读取就绪
- 如果确认整个列表中都没有 → ❌ 标记为缺失
- **常见误判：scope 列表很长时容易遗漏，务必搜索而非目视检查**
...
```
3. 脚本自动：获取 bot 所在群 → 找共同群 → 读取目标用户全部消息
