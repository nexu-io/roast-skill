...
当推特能力检测为 ❌ 时，**按以下顺序引导用户**：

**第 1 步：安装 Agent Reach**（让 agent 获得 twitter-cli 工具）
...
**第 2 步：获取 Twitter Cookie**（这是关键，很多用户卡在这里）

推特不提供公开 API，需要用浏览器 Cookie 来模拟登录。获取方法：

> **方法 A：用 Cookie-Editor 浏览器扩展（推荐，最简单）**
>
> 1. 安装浏览器扩展 [Cookie-Editor](https://cookie-editor.com/)（Chrome / Firefox / Edge 都支持）
> 2. 打开 [x.com](https://x.com) 并确保已登录
> 3. 点击 Cookie-Editor 图标 → 点击「Export」→ 选择「Header String」格式
> 4. 复制导出的内容，发给你的 agent，说：
>    ```
>    这是我的 Twitter Cookie，帮我配置一下：
>    <粘贴 Cookie 内容>
>    ```
> 5. Agent 会自动保存到 `~/.agent-reach/twitter-cookies.txt`
>
> **方法 B：浏览器开发者工具手动复制**
>
...
> 3. 刷新页面，点击任意一个请求
> 4. 在 Request Headers 里找到 `Cookie:` 这一行
> 5. 复制整行内容（很长，正常），发给 agent
>
> **方法 C：运行 `twitter auth`（如果 twitter-cli 已装好）**
>
> ```bash
> twitter auth
> ```
...
如果输出显示 `authenticated: true` + 你的用户名，就配置成功了 ✅

> ⚠️ **常见问题：**
> - Cookie 有有效期，一般 1-3 个月。如果某天突然报错，重新导出 Cookie 即可
> - 不需要 Twitter API Key / Bearer Token，纯 Cookie 就够
> - Cookie 只存在你本地 `~/.agent-reach/twitter-cookies.txt`，不会上传到任何地方
